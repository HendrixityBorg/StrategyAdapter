...
任一步失败都返回机器可读 `ContractError`，不得改用内置同名策略、替代引擎或宿主机执行。`psrc run --strategy-dir` 是单包统一入口，`psrc sandbox run --strategy-dir` 从宿主机把单包只读挂载进严格 Docker 边界；`psrc demo all` 只负责发现多个目录并逐个调用同一执行链路。直接入口支持 `--require-strict`；要求不能满足时返回 `SANDBOX_UNAVAILABLE`，禁止退回开发模式。
